...
GameStatus checkBoardStatus(Board board) {
    ...
    for (int row = 0; row < 3; row++) {
        if (board.value[row][0] == board.value[row][1] && 
            board.value[row][1] == board.value[row][2]) {
            if (board.value[row][0] == X) return X_WON;
            if (board.value[row][0] == O) return O_WON;
        }
    }

    // Sprawdzanie kolumn
    for (int col = 0; col < 3; col++) {
        if (board.value[0][col] == board.value[1][col] && 
            board.value[1][col] == board.value[2][col]) {
            if (board.value[0][col] == X) return X_WON;
            if (board.value[0][col] == O) return O_WON;
        }
    }

    // Sprawdzanie przekątnych
    if ((board.value[0][0] == board.value[1][1] && board.value[1][1] == board.value[2][2]) ||
        (board.value[0][2] == board.value[1][1] && board.value[1][1] == board.value[2][0])) {
        if (board.value[1][1] == X) return X_WON;
        if (board.value[1][1] == O) return O_WON;
    }
